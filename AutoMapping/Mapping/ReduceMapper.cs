...
namespace AutoMapping.Mapping
{
    public class ReducedAutoMapper
    {
        ...
        public static ReducedAutoMapper Instance => instance ?? (instance = new ReducedAutoMapper { mappingTypes = new Dictionary<object, object>() });

        public Dictionary<object, object> MappingTypes
        {
            get => mappingTypes;
            set => mappingTypes = value;
        }
            ...
            foreach (var currentRealProperty in properties)
            {
                ...
                else
                {
                    if (MappingTypes.ContainsKey(currentRealProperty.PropertyType) && shouldMapInnerEntities)
                    {
                        ...
                        var types = new[] { currentRealProperty.PropertyType, (Type)mapToObject };
                        ...
                        else if (currentDtoProperty.CanWrite)
                        {
        ...
        public List<TDestination> MapList<TSource, TDestination>(List<TSource> realObjects, Dictionary<object, object> alreadyInitializedObjects = null)
            ...
            where TDestination : class, new()
        {
            var mappedEntities = new List<TDestination>();
            foreach (var currentRealObject in realObjects)
            {
                var currentMappedItem = Map<TSource, TDestination>(currentRealObject, alreadyInitializedObjects: alreadyInitializedObjects);
